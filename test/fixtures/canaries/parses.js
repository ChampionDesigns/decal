 ...
 * handed to a guard at all, so every signature in Gate C has nothing to look at.
 *
 * Found on 27 August 2026 the way these always are: an agent hit the trap in an `html`
 * template, ran `npm run guards`, was told Gate C was OK, and lost a debugging cycle to a
 ...
 * `node --check` over it, and it is outside Gate C's own scan roots (src, styles, tools,
