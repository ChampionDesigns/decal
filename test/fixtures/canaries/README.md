...
Gate C's guards are tested against: "every guard ships with a canary — a fixture that
...
(SCOPE Part 8 §2, Gate C).

They are handed forward from Wave 0a item #2 (the base conventions) to item #4 (the
enforcement rig), which owns the guards themselves. Item #4 may add more.

| File | Violates | Guard it feeds |
|---|---|---|
| `colour-literal.js` | a raw colour literal in an authored `css` template | guard 3, the colour-literal guard (A8) |
| `important.js` | `!important` in component styles | LAYOUT_SPEC_DRAFT.md §2.1 Rule 3 |
| `font-face.js` | `@font-face` inside a component | Gate C rule 2 (spec §6.3 Rule 2) |
| `private-palette.js` | a `--ui-*` colour re-declared inside a component | guard 4 / bug L12 |
...
| `a8-source-text.js` | a test that reads the **source text** of a stylesheet and a component module and asserts on it — in all three shapes the scanner resolves (a literal in the read's own arguments, a local read helper, a local path binding walked by a `for…of`) | A8, `scripts/a8-source-text.js` (wave 5.5) |
...
**A8's guard does walk `test/`**, because tests are what it is about — so it carries
...
1. **Strip comments before scanning.** `src/components/base.js` writes the word
   ...
   Slate's `authored ... !important=yes` oracle lines. A guard that greps the raw
...
2. **Parse the `css` tagged templates, not just `.css` files** (Gate C's wording:
