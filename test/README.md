...
decision (A8). The current suite's 29 text-scanning files, 4,519 lines that never
...
Chrome over CDP (Gate A, Part 8 §2), fixed before the first component test is written.
Gate A's standard geometry is the bench truth: 1281x801 @ deviceScaleFactor 1.5,
...
`harness/` is the Gate A rig itself - headless Chrome over CDP, an ephemeral debug
...
`guards.test.mjs` is Gate C's canary suite - it asserts each build guard FAILS on its
...
`fixtures/canaries/` the files that violate a rule on purpose so each Gate C guard
...
`fixtures/dead-names/` is the same pair for the Gate 2 dead-name scan
(`rea-dead-names.test.mjs`): `reads-dead-name.js` reads names ReaPrime deleted, behind a
fallback that manufactures a plausible number — the exact shape the old skin shipped — and
...
`fixtures/excluded-surface/` is the same pair for the Gate 3 dead-surface scan
...
Every committed generated file has a suite that regenerates it and fails on a diff (SCOPE
Part 2 §7): `machine-state-freshness.test.mjs` for the enum, `rea-routes-freshness.test.mjs`
...
firmware stream. The two ReaPrime-side bugs with an executable signature (CB-05's cast before
the `try`, CB-06's `entries.first`) are re-proved rather than cited.

<!-- gate4-core-tests -->
## Gate 4 core — stores, shot buffer, time axis
...
* **Evidence at the pin** — `feed-readers.test.mjs` re-derives `ShotState`,
  ...
  pins B4's justification: both parsers stamp `DateTime.now()`, and `sampleTime` is decoded
