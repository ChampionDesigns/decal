...
surface**: no build step, no framework, no state model. Wave 0a item #4; Waves 1–4
add entries.

Open `tools/gallery/index.html` from any static server rooted at the repo (the Gate A
...
rather than the viewport (LAYOUT_SPEC_DRAFT §2.1 Rule 1). The viewport is the capture
...
a human review, not a diff (Gate B change 3).
...
* The importmap here mirrors `index.html`'s with `../../` paths — **same keys**,
  re-rooted values. `test/tools-port.test.mjs` asserts key-for-key equality, because
  ...
  in the Gate A harness and threw in the gallery. It was invisible because the one
  ...
  state, per theme, per geometry. (The Gate A harness does not have this problem: it
...
* The gallery is a separate document rather than a route, because the app is "one
  ...
  (SCOPE Part 2 §2). A tool does not belong in that document.
* Gate C scans this page. `tools/` and `index.html` are scan roots and the scanner
...
* The one entry that exists today is Wave 0a item #2's `base-fixture` — not a
