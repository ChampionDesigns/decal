...
`CONTRACTS.json` is the record required by SCOPE Part 3 §7 and enforced by Part 8 §2's
...
activity (DECISIONS.md). The check happens as the call is written, with the handler open
...
**The handler body is the authority.** Not `rest_v1.yml`, not `doc/Api.md`, not the old
skin's JSDoc — which asserts, for one example, that the cup warmer has "no separate enable
field" while the handler serves and validates one. Where the spec and the handler disagree
...
**A passing desk check is necessary, not sufficient.** The D7 LED episode is the standing
caution: static analysis was wrong in *both* directions across two passes, and Ben's
...
**The capture mock reads this same table** (Gate B rule 4). The handover this section once
...
fixture-derived `tools/mock-contract.json` and `mock_rea.contract_table_path()`'s
...
see `_skinlab/realine-run/waves/0b/mock-contract-mismatches.md`.
...
| `requestFields` | `{query, body, path}` — what the handler **reads**, not what the spec documents. `$`-prefixed keys are notes about the body as a whole. |
...
| `POST /machine/profile` | **400 `Unsupported profile` is the machine refusing.** `setProfile` is called outside `runDeviceWrite` specifically so the refusal is a clean 400 rather than the catch-all 500. B9: send the profile, surface the message; never pre-strip a step to dodge it. |
...
| `GET /shots/latest` | **200 carrying `null`** when nothing has ever been stored. That null is the answer (A7). |
...
| **staleness** | a row whose `checkedCommit` is not the pinned ReaPrime commit fails the build. ReaPrime is *expected* to move under this build (A2), so drift is a certainty, not a risk — the stamp turns "did anyone re-check?" into a diff. |
...
4. Where the re-check finds the **server** wrong, the fix goes upstream on a clean
   upstream-cut branch (A2) — never worked around in the skin. The four ReaPrime-side
   contract bugs are listed in `reaPrimeSide` and swept in
   `realine-run/waves/0b/contract-bug-sweep.md`.
