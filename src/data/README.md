...
current-names reader. Contract detail belongs to the contract table, not to the layout.

The storage-key prefix and every app-level event name follow the skin id: `decal.*`
(renamed with the id, no migration). Nothing here is named after another skin.
...
| `rea-profile.js` | the ONE profile sanitizer, and the refusal philosophy: send it, let ReaPrime return the typed 400, surface the message intact. |
...
| `rea-routes.js` | lookup and spelling over that table, plus the five call helpers the stores actually call. Table complete, helpers demand-driven **and consumer-checked** — see `ROUTES.md`. |
| `ROUTES.md` | the route layer's own doc: the split, the three named exceptions and what was checked rather than assumed. |
...
frame rate. Unifying them, not the wrapper count, is the work here (see
the transport notes).
...
| `rea-devices.js` | read the whole `connectionStatus` (phase, found lists, `pendingAmbiguity`, `error`) **and send the answer** over `PUT /api/v1/devices/connect`. A malformed frame is `null`, distinct from an empty list. |
...
semantics the stores build on. `test/rea-devices.test.mjs` covers the full frame,
...
machine refusing, `/shots` clamps the limit to 100 and echoes the number you asked for,
