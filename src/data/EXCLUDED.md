...
The dead surface of, listed so a later wave does not helpfully port it back.

 is 2,406 lines and **665 of them are DROP**: 274 defensive
...
that shape: 's preview and F3's reset are both *wanted*, and both are still excluded,
...
| `currentShotSettings`, `updateShotSettingsCache`, `sendShotSettings` | A write-only mirror kept current for a function with no caller anywhere in `src/` or `test/`. | |
...
| The profiles-to-IDB cache | Its only read is inside an API catch block that cannot fire: ReaPrime serves the skin and the API from one origin, so if the API is down, nothing served the skin. | |
...
| `connectProfileGeneratedWebSocket` | An entire 34-line WebSocket connector nobody opens. | |
| `getEstimatorLink`, `getDisplayWebSocket`, `getUpdateWebSocket`, `uploadMachineProfile`, `getDisplayState` | Five more zero-call-site exports, verified by grep across `src/` and `test/`. | |
...
| Not built | Why | Citation |
|---|---|---|
| `import * as ui from './'` | imports twenty names back — a cycle. The transport painted the machine-status label itself and raised toasts from inside upload failures. Nothing in `src/data/` imports from `src/components/`, `src/screens/` or `src/stores/`, and a test asserts it. | |
| The three `alert()`s inside `isValidProfile` | A transport module does not open a modal, and validation is the server's answer, not a guess made before sending. | |
| The five catch blocks that **manufacture an answer** | `getReaSettings` → `null`, `getPlugins` → `null`, `getPluginSettings` → `{}`, `verifyVisualizerCredentials` → `false`, `getScaleDeviceId` → `null`. The fourth is the clearest: an unreachable server becomes indistinguishable from a wrong password., and the reason failures here are returned as typed data. | `rea-errors.js` header. |
| `connectScaleDevice`'s catch block | `return response.json()` where `response` is `const`-scoped to the `try` — a `ReferenceError` sitting inside an error path, which is to say the error path has never once run to completion. | |
...
| The two hand-rolled retry timers | Retry is a policy, and the one place it belongs is the reconnecting socket, which encodes it correctly and was patched on the bench for exactly this. Nothing in the REST path retries. | DROP breakdown, 274 defensive lines. |

## Excluded because the work is elsewhere

| Not built | Why | Citation |
|---|---|---|
| and | Their founding premise — that ReaPrime bound machine sockets to a De1 *instance* and never re-bound them on swap — was fixed in ReaPrime `31ea7c1a`: `_withDe1Ws` detaches and re-attaches every machine-bound socket when the instance changes. Neither module ports. Recorded here as well as in its own item, because "the socket layer got simpler" is exactly the kind of change a later wave re-complicates. | Socket lifecycle is now mostly ReaPrime's problem; `de1handler.dart` `_withDe1Ws` `gate3-drop-socket-slot-machine-link`. |
| Reset-to-default, in any form — including `DELETE /api/v1/machine/settings/reset` | **F3/: no work of any kind.** The semantics are blocked on a decision. The route exists in the handler and is not called from this repo. `createDe1SettingsClient(..).invalidate` is public so that whoever builds it has the correct cache hook and does not invent a second one. | upstream item F3; `de1handler.dart` `DELETE /api/v1/machine/settings/reset`. |
