...
Not consumed, deliberately: *"`/ws/v1/machine/raw`, `/ws/v1/logs`,
...
* ** (75 lines)** — its founding premise was that ReaPrime bound machine
...
* ** (128 lines)** — drove the resync machinery that premise required.
   replaces it with something richer (`rea-devices.js`), and its one durable rule — a
...
`/ws/v1/machine/shotState` is consumed (it is in the table), but nothing in this layer derives
shot phase from it. That is this layer's one shot derivation. Likewise the shot-so-far buffer:
